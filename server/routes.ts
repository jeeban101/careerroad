...
import { validateResourceUrl } from "./resourceSecurity";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/resource-metadata", async (req, res) => {
    try {
      const { url } = req.body;
      ...
      // Security: Validate URL against allowlist and check for SSRF
      const validation = await validateResourceUrl(url);
      if (!validation.valid) {
        return res.status(400).json({ message: validation.reason || "URL not allowed" });
      }
      ...
      // Fetch metadata with manual redirect handling
      try {
        ...
        try {
          while (redirectCount <= MAX_REDIRECTS) {
            const response = await fetch(currentUrl, {
              headers: {
                "User-Agent": "Mozilla/5.0 (compatible; CareerRoad/1.0; +https://careerroad.app)"
              },
            ...
            if (response.status >= 300 && response.status < 400) {
              ...
              if (!location) {
                ...
                return res.status(502).json({ message: "Invalid redirect response" });
              }
              ...
              if (currentParsed.protocol === "https:" && redirectParsed.protocol === "http:") {
                console.error(`Blocked protocol downgrade: ${currentUrl} → ${redirectUrl}`);
                return res.status(400).json({ message: "Protocol downgrade not allowed" });
              }

              // Validate redirect target against allowlist and SSRF
              const redirectValidation = await validateResourceUrl(redirectUrl);
              if (!redirectValidation.valid) {
                console.error(`Blocked redirect: ${currentUrl} → ${redirectUrl} (${redirectValidation.reason})`);
                return res.status(400).json({ 
        ...
        if (!finalResponse.ok) {
          return res.status(finalResponse.status).json({ 
            message: `Failed to fetch URL: ${finalResponse.statusText}` 
          });
        }

        const xFrameOptions = finalResponse.headers.get("x-frame-options");
        const csp = finalResponse.headers.get("content-security-policy");
        
        // Check if embeddable
        const canEmbed = !xFrameOptions && (!csp || !csp.includes("frame-ancestors"));
        ...
        const docTitle = titleMatch ? titleMatch[1] : null;

        // Extract first paragraph for preview
        let previewHtml = "";
        if (resourceType === "article") {
          const pMatch = html.match(/<p[^>]*>([^<]*(?:<[^/](?!p>)[^<]*)*)<\/p>/i);
          if (pMatch) {
            previewHtml = sanitizeHtml(pMatch[0], {
        ...
        const description = ogDescription || (previewHtml ? previewHtml.substring(0, 400) : "");
        ...
        res.json({
          ...
          hostname,
          previewHtml: "",
          canEmbed: resourceType === "video" || resourceType === "interactive",
          url
        });
      }
    } catch (error) {
      console.error("Error fetching resource metadata:", error);
      res.status(500).json({ message: "Failed to fetch resource metadata" });
    }
  });
